...
def test_initialize_uvh5_file(tmpdir: str):
    ...
    pt_dec = get_config('pot_dec')
    antenna_order = np.random.shuffle(np.arange(1, nants+1))
    ...
    with h5py.File(f"{tmpdir}/test.hdf5", "w") as fhdf5:
        initialize_uvh5_file(fhdf5, nfreq, npol, pt_dec, antenna_order, fobs, fs_table)
    ...
    with h5py.File(f"{tmpdir}/test.hdf5", "r") as fhdf5:
        ...
        for key in [
                ...
                "uvw_array", "time_array", "integration_array", "integration_time", "freq_array",
...
def test_update_uvh5_file(tmpdir: str):
    ...
    data = np.ones((nt, nbl, nchan, npol), dtype=complex64)

    with h5py.File(f"{tmpdir}/test.hdf5", "a") as fhdf5:
        update_uvh5_file(fhdf5, data, obstime, tsamp, bname, uvw, nsamples)
